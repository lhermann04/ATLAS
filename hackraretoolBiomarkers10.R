  ...
  output$detection_prob <- renderText({
    ...
    p_values <- replicate(n_sim, {
      ...
      for(i in 1:n){
        ...
        baseline_vals <- rnorm(6, baseline_mean, top_sd)
        treatment_vals <- rnorm(6, treatment_mean, top_sd * noise)
    ...
    paste("Statistical Power:", round(power_estimate * 100, 1), "%")
  })
