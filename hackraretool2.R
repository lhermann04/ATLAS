...
ui <- fluidPage(
  ...
  div(class = "main-title",
      h1("🧬 TinyTrial: Community Genotype–Phenotype Engine")
  ),
  ...
  sidebarLayout(
    ...
    sidebarPanel(width = 3,
                 ...
                 div(class="card",
                     ...
                     helpText("Preferred template columns:"),
                     helpText("Patient_ID | Variant | Symptom | Severity | Date | Biomarker | Biomarker_Value")
                 ),
    ...
    mainPanel(width = 9,
              ...
              tabsetPanel(
                ...
                tabPanel("🧬 Genotype–Phenotype",
                         ...
                         div(class="card",
                             h4("Symptom Frequency by Variant"),
...
server <- function(input, output, session) {
  ...
  # Standardization layer (Template Mode)
  cleaned_data <- reactive({
    ...
    # Light fallback mapping
    colnames(df) <- gsub(" ", "_", colnames(df))
    
    # Ensure required columns exist
    required <- c("patient_id","variant","symptom",
                  "severity","date",
  ...
  output$variant_plot <- renderPlot({
    ...
    cleaned_data() %>%
      ...
      coord_flip() +
      theme_minimal() +
      labs(x="Variant", y="Patient Count")
  })
  ...
  sim_data <- eventReactive(input$run_sim,{
    ...
    top_biomarker <- biomarker_scores()$biomarker[1]
    
    baseline_sd <- biomarker_scores()$mean_within_sd[1]
    ...
    for(i in 1:n){
      baseline <- rnorm(6,10,baseline_sd)
      treatment <- rnorm(6,10*(1-effect),baseline_sd*noise)
